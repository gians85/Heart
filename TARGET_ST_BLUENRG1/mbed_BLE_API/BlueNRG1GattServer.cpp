...
*/
/**************************************************************************/
ble_error_t BlueNRG1GattServer::addService(GattService &service)
{
    ...
    if(type==UUID::UUID_TYPE_SHORT) {
        memcpy(&service_uuid.Service_UUID_16, primary_short_uuid, 2);
        ret = aci_gatt_add_service(UUID_TYPE_16,
                                   &service_uuid,
                                   PRIMARY_SERVICE,
                                   maxAttrRecords,
                                   &servHandle);
        PRINTF("aci_gatt_add_service UUID_TYPE_SHORT ret=%d\n\r", ret);

    } else if(type==UUID::UUID_TYPE_LONG) {
        memcpy(&service_uuid.Service_UUID_128, primary_base_uuid, 16);
        ret = aci_gatt_add_service(UUID_TYPE_128,
                                   &service_uuid,
                                   PRIMARY_SERVICE,
                                   maxAttrRecords,
                                   &servHandle);
        PRINTF("aci_gatt_add_service UUID_TYPE_LONG ret=%d\n\r", ret);
    }
    ...
    for (uint8_t i = 0; i < charsCount; i++) {
        ...
        uint16_t char_short_uuid = (p_char->getValueAttribute().getUUID()).getShortUUID();

        uint8_t int_8_uuid[2];
        STORE_LE_16(int_8_uuid, char_short_uuid);
        ...
        */
        ...
        if(type==UUID::UUID_TYPE_SHORT) {
            memcpy(&char_uuid.Char_UUID_16, int_8_uuid, 2);
            ret =  aci_gatt_add_char(service.getHandle(),
                                     UUID_TYPE_16,
                                     &char_uuid,
                                     p_char->getValueAttribute().getMaxLength() /*Value Length*/,
            ...
            memcpy(&char_uuid.Char_UUID_128, char_base_uuid, 16);
            ret =  aci_gatt_add_char(service.getHandle(),
                                     UUID_TYPE_128,
                                     &char_uuid,
                                     p_char->getValueAttribute().getMaxLength() /*Value Length*/,
        ...
        p_char->getValueAttribute().setHandle(bleCharacteristic+BlueNRG1GattServer::CHAR_VALUE_HANDLE);
        ...
        for(uint8_t descIndex=0; descIndex<p_char->getDescriptorCount(); descIndex++) {
            ...
            memcpy(&char_desc_uuid.Char_UUID_16, desc_uuid, 2);
            ...
            if((descriptor->getUUID()).shortOrLong() == UUID::UUID_TYPE_LONG) {
                ...
                memcpy(&char_desc_uuid.Char_UUID_128, base_desc_uuid, 16);
...
*/
/**************************************************************************/
ble_error_t BlueNRG1GattServer::read(GattAttribute::Handle_t attributeHandle, uint8_t buffer[], uint16_t *lengthP)
{
  tBleStatus ret;
  uint16_t charHandle = attributeHandle;
  uint16_t length = 0;

  ret = aci_gatt_read_handle_value(charHandle, 0, *lengthP, &length, lengthP, buffer);
...
*/
/**************************************************************************/
ble_error_t BlueNRG1GattServer::read(Gap::Handle_t connectionHandle,
...
ble_error_t BlueNRG1GattServer::write(Gap::Handle_t connectionHandle,
...
ble_error_t BlueNRG1GattServer::write(GattAttribute::Handle_t attributeHandle, const uint8_t buffer[], uint16_t len, bool localOnly)
{
    ...
    if (attributeHandle == characteristic->getValueHandle()) {
        ...
        uint16_t charHandle = characteristic->getValueHandle() - BlueNRG1GattServer::CHAR_VALUE_HANDLE;
...
*/
/**************************************************************************/
ble_error_t BlueNRG1GattServer::Read_Request_CB(uint16_t attributeHandle)
{
    uint16_t gapConnectionHandle = BlueNRG1Gap::getInstance().getConnectionHandle();
...
// case of faillure
uint8_t BlueNRG1GattServer::Write_Request_CB(
...
*/
/**************************************************************************/
GattCharacteristic* BlueNRG1GattServer::getCharacteristicFromHandle(uint16_t attrHandle)
{
    GattCharacteristic *p_char = NULL;
    int i;
    uint16_t handle, handle_1;

    PRINTF("BlueNRG1GattServer::getCharacteristicFromHandle()>>Attr Handle received %d\n\r",attrHandle);
    for(i=0; i<characteristicCount; i++)
    {
        handle = p_characteristics[i]->getValueAttribute().getHandle()-BlueNRG1GattServer::CHAR_VALUE_HANDLE;
        PRINTF("handle(%d)=%d\n\r", i, handle);
        if(i==characteristicCount-1)//Last Characteristic check
        {
            ...
            handle_1 = p_characteristics[i+1]->getValueAttribute().getHandle()-BlueNRG1GattServer::CHAR_VALUE_HANDLE;
...
void BlueNRG1GattServer::HCIDataWrittenEvent(const GattWriteCallbackParams *params) {
...
void BlueNRG1GattServer::HCIEvent(GattServerEvents::gattEvent_e type, uint16_t charHandle) {
    this->handleEvent(type, charHandle);
}

void BlueNRG1GattServer::HCIDataSentEvent(unsigned count) {
    this->handleDataSentEvent(count);
}


ble_error_t BlueNRG1GattServer::initializeGATTDatabase(void)   {
    // <TODO>
...
/**************************************************************************/
/*!
    @brief  Clear BlueNRG1GattServer's state.
...
*/
/**************************************************************************/
ble_error_t BlueNRG1GattServer::reset(void)
{
    PRINTF("BlueNRG1GattServer::reset\n");
...
uint16_t BlueNRG1GattServer::computeAttributesRecord(GattService& service) {
