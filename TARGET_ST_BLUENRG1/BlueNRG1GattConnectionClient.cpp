...
/**
  ******************************************************************************
  * @file    BlueNRG1GattConnectionClient.cpp
...
/** @defgroup BlueNRG1GattConnectionClient
...
void BlueNRG1GattConnectionClient::gattProcedureCompleteCB(uint8_t error_code)
{
...
*/
  ...
  if(_currentState == GATT_READ_CHAR) {
    _currentState = GATT_IDLE;
    BlueNRG1GattClient::getInstance().processReadResponse(&readCBParams);
  ...
  if(_currentState == GATT_WRITE_CHAR) {
    _currentState = GATT_IDLE;
    BlueNRG1GattClient::getInstance().processWriteResponse(&writeCBParams);
    return;
  }
}

void BlueNRG1GattConnectionClient::primaryServicesCB(uint8_t event_data_length,
...
void BlueNRG1GattConnectionClient::primaryServiceCB(uint8_t numHandlePairs/*event_data_length*/, uint8_t *handles_info_list)
{
  GattAttribute::Handle_t startHandle, endHandle;
  UUID uuid;
...
void BlueNRG1GattConnectionClient::serviceCharsCB(uint8_t event_data_length,
...
void BlueNRG1GattConnectionClient::serviceCharByUUIDCB(uint8_t event_data_length,
                                                      ...
                                                      uint8_t *attr_value)
{
  ...
  PRINTF("serviceCharByUUIDCB (event_data_length=%d)\n\r", event_data_length);

  // UUID Type
  if (event_data_length == 5) {
    PRINTF("Char UUID_TYPE_16\n\r");
...
ble_error_t BlueNRG1GattConnectionClient::findServiceChars(void)
{
  ...
  */
  ...
  if(_matchingCharacteristicUUIDIn == BLE_UUID_UNKNOWN) {
      ...
      ret = aci_gatt_disc_all_char_of_service(_connectionHandle, service->getStartHandle(), service->getEndHandle());
  } else {
      ...
      if(type == UUID::UUID_TYPE_SHORT) {
          ...
          short_uuid = _matchingCharacteristicUUIDIn.getShortUUID();
#ifdef DEBUG
          PRINTF("findServiceChars C UUID-");
          for(unsigned i = 0; i < 2; i++) {
              PRINTF("%02X", (uint8_t)(short_uuid>>(i*8)));
          }
          PRINTF("\n\r");
#endif

          memcpy(&char_uuid.UUID_16, &short_uuid, 2);

      } else if(type==UUID::UUID_TYPE_LONG) {

          uuid_type = UUID_TYPE_128;
          uuid = (uint8_t*)_matchingCharacteristicUUIDIn.getBaseUUID();
#ifdef DEBUG
          PRINTF("(findServiceChars) C UUID-");
          for (unsigned i = 0; i < UUID::LENGTH_OF_LONG_UUID; i++) {
              PRINTF("%02X", uuid[i]);
          ...
          memcpy(&char_uuid.UUID_128, uuid, 16);

      }

      ret = aci_gatt_disc_char_by_uuid(_connectionHandle,
...
ble_error_t BlueNRG1GattConnectionClient::launchServiceDiscovery(ServiceDiscovery::ServiceCallback_t         sc,
                                                                ServiceDiscovery::CharacteristicCallback_t  cc,
                                                                const UUID                                 &matchingServiceUUID,
                                                                const UUID                                 &matchingCharacteristicUUIDIn)
{
  PRINTF("BlueNRG1GattConnectionClient launchServiceDiscovery\n\r");
  ...
  if(!sc && !cc) {
    // nothing to do
    PRINTF("BlueNRG1GattConnectionClient launchServiceDiscovery: nothing to do\n\r");
    return BLE_ERROR_NONE;
  }
  ...
  if(matchingServiceUUID == BLE_UUID_UNKNOWN) {
    ...
    ret = aci_gatt_disc_all_primary_services((uint16_t)_connectionHandle);
    ...
    if(type == UUID::UUID_TYPE_SHORT) {
      ...
      uuid_type = UUID_TYPE_16;
      short_uuid = matchingServiceUUID.getShortUUID();
#ifdef DEBUG
      PRINTF("BlueNRG1GattConnectionClient launchServiceDiscovery short_uuid=0x");
      for(j = 0; j < 2; j++) {
        PRINTF("%02X", (uint8_t)(short_uuid>>(j*8)));
      }
      PRINTF("\n\r");
#endif

      memcpy(&service_uuid.UUID_16, &short_uuid, 2);

    } else if(type==UUID::UUID_TYPE_LONG) {

      uuid_type = UUID_TYPE_128;
      uuid = (uint8_t *)matchingServiceUUID.getBaseUUID();

#ifdef DEBUG
      PRINTF("BlueNRG1GattConnectionClient launchServiceDiscovery base_uuid=0x");
      for(j = 0; j < 16; j++) {
        PRINTF("%02X", uuid[j]);
      }
      PRINTF("\n\r");
      ...
      memcpy(&service_uuid.UUID_128, uuid, 16);
    }

    // search for specific service by UUID
    ret = aci_gatt_disc_primary_service_by_uuid((uint16_t)_connectionHandle, uuid_type, &service_uuid);
  }
  ...
  PRINTF("BlueNRG1GattConnectionClient launchServiceDiscovery ret=%d\n\r", ret);

  return BLE_ERROR_NONE;
}

ble_error_t BlueNRG1GattConnectionClient::discoverServices(ServiceDiscovery::ServiceCallback_t  callback,
                                                          const UUID                          &matchingServiceUUID)
{
...
bool BlueNRG1GattConnectionClient::isServiceDiscoveryActive(void) const
{
  if(_currentState == GATT_SERVICE_DISCOVERY) {
    return true;
  }
...
void BlueNRG1GattConnectionClient::terminateServiceDiscovery(void)
{
...
void BlueNRG1GattConnectionClient::charReadCB(uint8_t event_data_length, uint8_t* attribute_value)
{
...
ble_error_t BlueNRG1GattConnectionClient::read(GattAttribute::Handle_t attributeHandle, uint16_t offset) const
{
  /* avoid compiler warnings about unused variables */
  (void)offset;
  ...
  BlueNRG1GattConnectionClient *gattc = const_cast<BlueNRG1GattConnectionClient*>(this);

  // Save the attribute_handle not provided by evt_att_read_resp
  gattc->readCBParams.handle = attributeHandle;

  ret = aci_gatt_read_char_value(_connectionHandle, attributeHandle);
...
void BlueNRG1GattConnectionClient::charWritePrepareCB(uint8_t event_data_length,
...
void BlueNRG1GattConnectionClient::charWriteExecCB(void)
{
...
ble_error_t BlueNRG1GattConnectionClient::write(GattClient::WriteOp_t    cmd,
                                     ...
                                     const uint8_t           *value) const
{
  ...
  ret = aci_gatt_write_char_value(_connectionHandle, attributeHandle, length, const_cast<uint8_t *>(value));
...
void BlueNRG1GattConnectionClient::discAllCharacDescCB(uint8_t event_data_length,
                                                      uint8_t format,
...
ble_error_t BlueNRG1GattConnectionClient::discoverCharacteristicDescriptors(
  ...
  PRINTF("Starting aci_gatt_disc_all_char_desc... [%u : %u]\n\r", valueHandle, lastHandle);
  ret = aci_gatt_disc_all_char_desc(_connectionHandle, valueHandle, lastHandle);
...
*/
/**************************************************************************/
ble_error_t BlueNRG1GattConnectionClient::reset(void) {
  PRINTF("BlueNRG1GattConnectionClient::reset\n");
